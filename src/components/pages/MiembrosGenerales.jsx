...
export default function MiembrosGenerales() {
  ...
  const [selectedCulto, setSelectedCulto] = useState('domingo-noche')
  ...
  const loadMiembros = async () => {
    setLoading(true)
    try {
      const data = await getPersonas('miembros')
      setMiembros(data)
    } catch (error) {
      console.error('Error al cargar miembros:', error)
      alert('Error al cargar miembros. Ver consola.')
    } finally {
      setLoading(false)
    }
  }

  const handleAddMiembro = async (nombre, email, telefono) => {
    setLoading(true)
    try {
  ...
  const handleSaveAttendance = async (attendance) => {
    ...
    try {
      ...
      if (resultado.success) {
        alert(`✅ Asistencia registrada: ${attendance.length} personas`)
        setShowForm(false)
      } else {
        alert(`❌ Error: ${resultado.error}`)
      }
    ...
    if (qrAttendance.length > 0) {
      ...
      try {
        ...
        if (resultado.success) {
          ...
          alert(`✅ Asistencia guardada: ${qrAttendance.length} personas registradas`)
        } else {
          alert(`❌ Error: ${resultado.error}`)
        }
      } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-300">Cargando miembros...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-4xl font-display font-bold bg-gradient-to-r from-emerald-400 via-teal-400 to-emerald-500 bg-clip-text text-transparent">
            Miembros Generales
          </h2>
          <p className="text-gray-400 mt-2 text-lg">Gestión de asistencia a cultos regulares</p>
        </div>
      ...
      {showForm && (
        <div className="card p-6 md:p-8 animate-slide-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-300 mb-2">Tipo de Culto</label>
              <select
                value={selectedCulto}
      ...
      {showQRScanner && (
        <QRScanner
          ...
          onClose={() => {
            if (qrAttendance.length > 0) {
              if (confirm(`Tienes ${qrAttendance.length} personas escaneadas. ¿Deseas guardar la asistencia antes de cerrar?`)) {
